(function(){"use strict";var I;const A={apiBaseUrl:"https://botbackend.pentarixlabs.com",companyId:"",title:"Support Chat",subtitle:"Ask from our knowledge base",accentColor:"#111827",headerColor:"",headerTextColor:"",sendButtonColor:"",launcherColor:"",launcherIcon:"bot",position:"right",apiKey:""};function y(e){return`rag_widget_session_${e}`}function C(e){const r=`web_${crypto.randomUUID()}`;return localStorage.setItem(y(e),r),r}function T(e){const r=y(e);let o=localStorage.getItem(r);return o||(o=C(e)),o}function u(e,r){return typeof e=="string"&&/^#[0-9a-fA-F]{6}$/.test(e)?e:r}function $(e){return e==="question"?"?":e==="message"?`
  ...
  `}function q(e){const r=document.createElement("style"),o=e.position==="left"?"left":"right",t=u(e.accentColor,"#111827"),a=u(e.headerColor,t),n=u(e.headerTextColor,"#ffffff"),s=u(e.sendButtonColor,t),i=u(e.launcherColor,t);r.textContent=`
  ...
  `,document.head.appendChild(r)}function v(e,r){const o=/(\*\*[^*]+\*\*|\*[^*]+\*)/g;for(const t of r.split(o).filter(Boolean))if(t.startsWith("**")&&t.endsWith("**")){const a=document.createElement("strong");a.textContent=t.slice(2,-2),e.appendChild(a)}else if(t.startsWith("*")&&t.endsWith("*")){const a=document.createElement("em");a.textContent=t.slice(1,-1),e.appendChild(a)}else e.appendChild(document.createTextNode(t))}function N(e,r){let o=null;for(const t of String(r||"").split(`
`)){const a=t.match(/^\s*[-*]\s+(.+)/);if(a){o||(o=document.createElement("ul"),o.className="ragw-list",e.appendChild(o));const s=document.createElement("li");v(s,a[1]),o.appendChild(s);continue}o=null;const n=document.createElement("div");n.className="ragw-line",v(n,t),e.appendChild(n)}}function d(e,r,o=[],t=null,a=[]){const n=document.createElement("div");if(n.className=`ragw-msg ${e==="user"?"ragw-user":"ragw-bot"}`,N(n,r),o.length){const s=document.createElement("div");s.className="ragw-sources",s.textContent=`Sources: ${o.map(i=>i.documentName).filter(Boolean).join(", ")}`,n.appendChild(s)}if(e!=="user"&&a.length){const s=document.createElement("div");s.className="ragw-suggestions";for(const i of a){const c=document.createElement("button");c.type="button",c.className="ragw-suggestion",c.textContent=i.label,c.addEventListener("click",()=>{var w;s.querySelectorAll("button").forEach(x=>{x.disabled=!0}),(w=t==null?void 0:t.onSuggestion)==null||w.call(t,i.message,i.label)}),s.appendChild(c)}n.appendChild(s)}return n}function S(){const e=document.createElement("div");return e.className="ragw-msg ragw-bot ragw-typing",e.setAttribute("aria-label","Assistant is typing"),e.innerHTML='<span class="ragw-dot"></span><span class="ragw-dot"></span><span class="ragw-dot"></span>',e}async function M(e,r,o,{isSuggestion:t=!1}={}){const a=await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-Widget-API-Key":e.apiKey},body:JSON.stringify({message:r,sessionId:o,customerName:e.customerName||"",customerEmail:e.customerEmail||"",customerPhone:e.customerPhone||"",isSuggestion:t})}),n=await a.json();if(!a.ok)throw new Error(n.error||"Chat request failed");return n}async function L(e,r){const o=await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat/history/${encodeURIComponent(r)}`,{headers:{"X-Widget-API-Key":e.apiKey}}),t=await o.json();if(!o.ok)throw new Error(t.error||"Chat history request failed");return t}function f(e={}){const r={...A,...window.RAG_CHAT_WIDGET,...e};if(!r.companyId){console.error("[RAG Widget] companyId is required");return}q(r);let o=T(r.companyId);const t=document.createElement("div");t.className="ragw-root",t.innerHTML=`
    ...
    <button class="ragw-button" type="button" aria-label="Open chat">${$(r.launcherIcon)}</button>
  `,t.querySelector(".ragw-title").textContent=r.title,t.querySelector(".ragw-subtitle").textContent=r.subtitle;const a=t.querySelector(".ragw-messages"),n=t.querySelector(".ragw-input"),s=t.querySelector(".ragw-form"),i=t.querySelector(".ragw-send"),c=t.querySelector(".ragw-button"),w=t.querySelector(".ragw-new-chat"),x=t.querySelector(".ragw-close");let E=!1;n.disabled=!0,i.disabled=!0;async function k(l,g=l,{isSuggestion:m=!1}={}){if(!l||!E)return;n.value="",a.appendChild(d("user",g)),a.scrollTop=a.scrollHeight,i.disabled=!0;const p=S();a.appendChild(p),a.scrollTop=a.scrollHeight;const b=o;try{const h=await M(r,l,b,{isSuggestion:m});if(o!==b)return;p.remove(),a.appendChild(d("bot",h.answer,h.sources||[],{onSuggestion:(W,z)=>k(W,z,{isSuggestion:!0})},h.suggestions||[]))}catch(h){if(o!==b)return;p.remove(),a.appendChild(d("bot",h.message||"Unable to send message."))}finally{i.disabled=!1,a.scrollTop=a.scrollHeight}}c.addEventListener("click",()=>t.classList.toggle("ragw-open")),w.addEventListener("click",()=>{o=C(r.companyId),a.replaceChildren(d("bot",r.greeting||"Hi, how can I help?")),n.value="",n.focus()}),x.addEventListener("click",()=>t.classList.remove("ragw-open")),s.addEventListener("submit",async l=>{l.preventDefault(),await k(n.value.trim())}),document.body.appendChild(t);async function H(){const l=S();a.appendChild(l);try{const g=await L(r,o);l.remove();const m=Array.isArray(g.messages)?g.messages:[];if(!m.length){a.appendChild(d("bot",r.greeting||"Hi, how can I help?"));return}for(const p of m)a.appendChild(d(p.role,p.content,p.sources||[]))}catch(g){l.remove(),console.warn("[RAG Widget] Unable to restore chat history",g),a.appendChild(d("bot","I couldn't load your earlier messages, but you can start a new chat here."))}finally{E=!0,n.disabled=!1,i.disabled=!1,a.scrollTop=a.scrollHeight}}H()}window.RAGChatWidget={init:f},((I=window.RAG_CHAT_WIDGET)==null?void 0:I.autoInit)!==!1&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>f()):f())})();
