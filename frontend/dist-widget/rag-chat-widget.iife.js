...
`)){const o=t.match(/^\s*[-*]\s+(.+)/);if(o){r||(r=document.createElement("ul"),r.className="ragw-list",e.appendChild(r));const n=document.createElement("li");y(n,o[1]),r.appendChild(n);continue}r=null;const s=document.createElement("div");s.className="ragw-line",y(s,t),e.appendChild(s)}}function u(e,a,r=[],t=null,o=[]){const s=document.createElement("div");if(s.className=`ragw-msg ${e==="user"?"ragw-user":"ragw-bot"}`,$(s,a),r.length){const n=document.createElement("div");n.className="ragw-sources",n.textContent=`Sources: ${r.map(i=>i.documentName).filter(Boolean).join(", ")}`,s.appendChild(n)}if(e!=="user"&&o.length){const n=document.createElement("div");n.className="ragw-suggestions";for(const i of o){const c=document.createElement("button");c.type="button",c.className="ragw-suggestion",c.textContent=i.label,c.addEventListener("click",()=>{var d;n.querySelectorAll("button").forEach(h=>{h.disabled=!0}),(d=t==null?void 0:t.onSuggestion)==null||d.call(t,i.message,i.label)}),n.appendChild(c)}s.appendChild(n)}if(e!=="user"&&(t!=null&&t.showFeedback)&&o.length===0){const n=document.createElement("div");n.className="ragw-feedback",n.appendChild(document.createTextNode("Was this helpful?"));for(const[i,c]of[["helpful","👍 Yes"],["not_helpful","👎 No"]]){const d=document.createElement("button");d.type="button",d.textContent=c,d.addEventListener("click",async()=>{n.querySelectorAll("button").forEach(h=>h.classList.remove("ragw-selected")),d.classList.add("ragw-selected"),await t.onFeedback(i)}),n.appendChild(d)}s.appendChild(n)}return s}function C(){const e=document.createElement("div");return e.className="ragw-msg ragw-bot ragw-typing",e.setAttribute("aria-label","Assistant is typing"),e.innerHTML='<span class="ragw-dot"></span><span class="ragw-dot"></span><span class="ragw-dot"></span>',e}async function N(e,a,r){const t=await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-Widget-API-Key":e.apiKey},body:JSON.stringify({message:a,sessionId:r,customerName:e.customerName||"",customerEmail:e.customerEmail||"",customerPhone:e.customerPhone||""})}),o=await t.json();if(!t.ok)throw new Error(o.error||"Chat request failed");return o}async function q(e,a){const r=await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat/history/${encodeURIComponent(a)}`,{headers:{"X-Widget-API-Key":e.apiKey}}),t=await r.json();if(!r.ok)throw new Error(t.error||"Chat history request failed");return t}async function v(e,a,r){a&&await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat/feedback`,{method:"POST",headers:{"Content-Type":"application/json","X-Widget-API-Key":e.apiKey},body:JSON.stringify({conversationId:a,feedback:r})})}function b(e={}){const a={...S,...window.RAG_CHAT_WIDGET,...e};if(!a.companyId){console.error("[RAG Widget] companyId is required");return}T(a);const r=I(a.companyId),t=document.createElement("div");t.className="ragw-root",t.innerHTML=`
  ...
  `,t.querySelector(".ragw-title").textContent=a.title,t.querySelector(".ragw-subtitle").textContent=a.subtitle;const o=t.querySelector(".ragw-messages"),s=t.querySelector(".ragw-input"),n=t.querySelector(".ragw-form"),i=t.querySelector(".ragw-send"),c=t.querySelector(".ragw-button"),d=t.querySelector(".ragw-close");let h=!1;s.disabled=!0,i.disabled=!0;async function E(p,g=p){if(!p||!h)return;s.value="",o.appendChild(u("user",g)),o.scrollTop=o.scrollHeight,i.disabled=!0;const m=C();o.appendChild(m),o.scrollTop=o.scrollHeight;try{const l=await N(a,p,r);m.remove(),o.appendChild(u("bot",l.answer,l.sources||[],{showFeedback:a.showFeedback,onFeedback:w=>v(a,l.conversationId,w),onSuggestion:(w,x)=>E(w,x)},l.suggestions||[]))}catch(l){m.remove(),o.appendChild(u("bot",l.message||"Unable to send message."))}finally{i.disabled=!1,o.scrollTop=o.scrollHeight}}c.addEventListener("click",()=>t.classList.toggle("ragw-open")),d.addEventListener("click",()=>t.classList.remove("ragw-open")),n.addEventListener("submit",async p=>{p.preventDefault(),await E(s.value.trim())}),document.body.appendChild(t);async function L(){const p=C();o.appendChild(p);try{const g=await q(a,r);p.remove();const m=Array.isArray(g.messages)?g.messages:[];if(!m.length){o.appendChild(u("bot",a.greeting||"Hi, how can I help?"));return}for(const l of m){const w=l.role==="assistant";o.appendChild(u(l.role,l.content,l.sources||[],w?{showFeedback:a.showFeedback,onFeedback:x=>v(a,g._id,x)}:null))}}catch(g){p.remove(),console.warn("[RAG Widget] Unable to restore chat history",g),o.appendChild(u("bot","I couldn't load your earlier messages, but you can start a new chat here."))}finally{h=!0,s.disabled=!1,i.disabled=!1,o.scrollTop=o.scrollHeight}}L()}window.RAGChatWidget={init:b},((k=window.RAG_CHAT_WIDGET)==null?void 0:k.autoInit)!==!1&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>b()):b())})();
