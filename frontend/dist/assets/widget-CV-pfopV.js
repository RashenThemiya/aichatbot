const A={apiBaseUrl:"https://botbackend.pentarixlabs.com",companyId:"",title:"Support Chat",subtitle:"Ask from our knowledge base",accentColor:"#111827",headerColor:"",headerTextColor:"",sendButtonColor:"",launcherColor:"",launcherIcon:"bot",position:"right",apiKey:"",showFeedback:!1};function T(e){const a=`rag_widget_session_${e}`;let r=localStorage.getItem(a);return r||(r=`web_${crypto.randomUUID()}`,localStorage.setItem(a,r)),r}function w(e,a){return typeof e=="string"&&/^#[0-9a-fA-F]{6}$/.test(e)?e:a}function $(e){return e==="question"?"?":e==="message"?`
  ...
  `}function N(e){const a=document.createElement("style"),r=e.position==="left"?"left":"right",t=w(e.accentColor,"#111827"),o=w(e.headerColor,t),s=w(e.headerTextColor,"#ffffff"),n=w(e.sendButtonColor,t),i=w(e.launcherColor,t);a.textContent=`
  ...
  `,document.head.appendChild(a)}function C(e,a){const r=/(\*\*[^*]+\*\*|\*[^*]+\*)/g;for(const t of a.split(r).filter(Boolean))if(t.startsWith("**")&&t.endsWith("**")){const o=document.createElement("strong");o.textContent=t.slice(2,-2),e.appendChild(o)}else if(t.startsWith("*")&&t.endsWith("*")){const o=document.createElement("em");o.textContent=t.slice(1,-1),e.appendChild(o)}else e.appendChild(document.createTextNode(t))}function q(e,a){let r=null;for(const t of String(a||"").split(`
`)){const o=t.match(/^\s*[-*]\s+(.+)/);if(o){r||(r=document.createElement("ul"),r.className="ragw-list",e.appendChild(r));const n=document.createElement("li");C(n,o[1]),r.appendChild(n);continue}r=null;const s=document.createElement("div");s.className="ragw-line",C(s,t),e.appendChild(s)}}function m(e,a,r=[],t=null,o=[]){const s=document.createElement("div");if(s.className=`ragw-msg ${e==="user"?"ragw-user":"ragw-bot"}`,q(s,a),r.length){const n=document.createElement("div");n.className="ragw-sources",n.textContent=`Sources: ${r.map(i=>i.documentName).filter(Boolean).join(", ")}`,s.appendChild(n)}if(e!=="user"&&o.length){const n=document.createElement("div");n.className="ragw-suggestions";for(const i of o){const d=document.createElement("button");d.type="button",d.className="ragw-suggestion",d.textContent=i.label,d.addEventListener("click",()=>{var l;n.querySelectorAll("button").forEach(h=>{h.disabled=!0}),(l=t==null?void 0:t.onSuggestion)==null||l.call(t,i.message,i.label)}),n.appendChild(d)}s.appendChild(n)}if(e!=="user"&&(t!=null&&t.showFeedback)&&o.length===0){const n=document.createElement("div");n.className="ragw-feedback",n.appendChild(document.createTextNode("Was this helpful?"));for(const[i,d]of[["helpful","👍 Yes"],["not_helpful","👎 No"]]){const l=document.createElement("button");l.type="button",l.textContent=d,l.addEventListener("click",async()=>{n.querySelectorAll("button").forEach(h=>h.classList.remove("ragw-selected")),l.classList.add("ragw-selected"),await t.onFeedback(i)}),n.appendChild(l)}s.appendChild(n)}return s}function v(){const e=document.createElement("div");return e.className="ragw-msg ragw-bot ragw-typing",e.setAttribute("aria-label","Assistant is typing"),e.innerHTML='<span class="ragw-dot"></span><span class="ragw-dot"></span><span class="ragw-dot"></span>',e}async function L(e,a,r,{isSuggestion:t=!1}={}){const o=await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-Widget-API-Key":e.apiKey},body:JSON.stringify({message:a,sessionId:r,customerName:e.customerName||"",customerEmail:e.customerEmail||"",customerPhone:e.customerPhone||"",isSuggestion:t})}),s=await o.json();if(!o.ok)throw new Error(s.error||"Chat request failed");return s}async function F(e,a){const r=await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat/history/${encodeURIComponent(a)}`,{headers:{"X-Widget-API-Key":e.apiKey}}),t=await r.json();if(!r.ok)throw new Error(t.error||"Chat history request failed");return t}async function k(e,a,r){a&&await fetch(`${e.apiBaseUrl}/widget/companies/${e.companyId}/chat/feedback`,{method:"POST",headers:{"Content-Type":"application/json","X-Widget-API-Key":e.apiKey},body:JSON.stringify({conversationId:a,feedback:r})})}function x(e={}){const a={...A,...window.RAG_CHAT_WIDGET,...e};if(!a.companyId){console.error("[RAG Widget] companyId is required");return}N(a);const r=T(a.companyId),t=document.createElement("div");t.className="ragw-root",t.innerHTML=`
    ...
    <button class="ragw-button" type="button" aria-label="Open chat">${$(a.launcherIcon)}</button>
  `,t.querySelector(".ragw-title").textContent=a.title,t.querySelector(".ragw-subtitle").textContent=a.subtitle;const o=t.querySelector(".ragw-messages"),s=t.querySelector(".ragw-input"),n=t.querySelector(".ragw-form"),i=t.querySelector(".ragw-send"),d=t.querySelector(".ragw-button"),l=t.querySelector(".ragw-close");let h=!1;s.disabled=!0,i.disabled=!0;async function y(c,u=c,{isSuggestion:b=!1}={}){if(!c||!h)return;s.value="",o.appendChild(m("user",u)),o.scrollTop=o.scrollHeight,i.disabled=!0;const p=v();o.appendChild(p),o.scrollTop=o.scrollHeight;try{const g=await L(a,c,r,{isSuggestion:b});p.remove(),o.appendChild(m("bot",g.answer,g.sources||[],{showFeedback:a.showFeedback,onFeedback:f=>k(a,g.conversationId,f),onSuggestion:(f,I)=>y(f,I,{isSuggestion:!0})},g.suggestions||[]))}catch(g){p.remove(),o.appendChild(m("bot",g.message||"Unable to send message."))}finally{i.disabled=!1,o.scrollTop=o.scrollHeight}}d.addEventListener("click",()=>t.classList.toggle("ragw-open")),l.addEventListener("click",()=>t.classList.remove("ragw-open")),n.addEventListener("submit",async c=>{c.preventDefault(),await y(s.value.trim())}),document.body.appendChild(t);async function S(){const c=v();o.appendChild(c);try{const u=await F(a,r);c.remove();const b=Array.isArray(u.messages)?u.messages:[];if(!b.length){o.appendChild(m("bot",a.greeting||"Hi, how can I help?"));return}for(const p of b){const g=p.role==="assistant";o.appendChild(m(p.role,p.content,p.sources||[],g?{showFeedback:a.showFeedback,onFeedback:f=>k(a,u._id,f)}:null))}}catch(u){c.remove(),console.warn("[RAG Widget] Unable to restore chat history",u),o.appendChild(m("bot","I couldn't load your earlier messages, but you can start a new chat here."))}finally{h=!0,s.disabled=!1,i.disabled=!1,o.scrollTop=o.scrollHeight}}S()}window.RAGChatWidget={init:x};var E;((E=window.RAG_CHAT_WIDGET)==null?void 0:E.autoInit)!==!1&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>x()):x());
