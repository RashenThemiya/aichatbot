...
async function preprocessUserMessage(message, { skipSmallTalk = false } = {}) {
  const original = String(message || "").trim();
  if (!original) {
    return { type: "empty" };
  }

  // Suggestion payloads are generated by our own support flow. They can begin
  // with phrases such as "I am concerned...", which must not be parsed as a
  // customer's name or as unrelated social conversation.
  if (skipSmallTalk) {
    const result = { type: "support", question: original };
